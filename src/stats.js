// src/stats.js — compute stats from round history
...
export async function getMyStats(uid) {
  ...
  const totalPenalties = rounds.reduce((a, r) => a + r.penalties, 0);

  // Hole-level stats (fairways & GIR)
  ...
  // Handicap estimate (last 20 rounds)
  const recent = rounds
  ...
  // Trend (last 10 rounds chronologically)
  ...
  return { totalRounds, avgScore, bestRound, worstRound, avgPutts,
           totalPenalties, fairwayPct, girPct, handicap, trend };
}
